...
# Note: Please modify the following variables as needed.

# Variable to toggle between Atlanta and Texas
city = "Texas"  # Change this to "Atlanta" when needed

# Variable to hold the time range for data export
time_range = "week"  # Change this to "day", "week", "month", or "year"
...
# Automatically select the segments and data source based on the city
if city == "Atlanta":
    selected_segments = Atlanta
    selected_data_source = data_sources["Atlanta"]
elif city == "Texas":
    selected_segments = Texas
    selected_data_source = data_sources["Texas"]
...
def get_dates(time_range):
    ...
    if time_range == "day":
        ...
        raise ValueError("Invalid time range. Use 'day', 'week', 'month', or 'year'.")
...
def submit_export_job():
    ...
    start_date, end_date = get_dates(time_range)

    params = {
        "uuid": job_uuid,
        "segments": {"type": "tmc", "ids": selected_segments},
        "dates": [{"start": start_date, "end": end_date}],
        "dow": [0, 1, 2, 3, 4, 5, 6],
        "dsFields": [{"id": selected_data_source, "columns": all_columns}],
    ...
    print(f'\n\nWe are in {city}, using {selected_data_source}\n\n')
